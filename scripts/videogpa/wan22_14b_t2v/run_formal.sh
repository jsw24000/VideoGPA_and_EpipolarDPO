...
vgm_require_profile
CONFIG="${VGM_REPO_ROOT}/configs/videogpa/wan22_14b_t2v_formal.yaml"
RUN_ID=""
RESUME=0
FORCE_STAGE=""
STOP_AFTER=""
...
if [[ -d "${RUN_DIR}" && "${RESUME}" != "1" ]] && find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit | grep -q .; then
  printf '[run_formal_14b_t2v] refusing to reuse non-empty RUN_DIR without --resume: %s\n' "${RUN_DIR}" >&2
  exit 2
fi
...
run_stage() {
  ...
  if [[ -f "${done_marker}" && "${FORCE_STAGE}" != "${stage}" ]]; then
    printf '[run_formal_14b_t2v] skip done stage %s\n' "${stage}"
    return
  fi
  printf '[run_formal_14b_t2v] start stage %s\n' "${stage}"
  state_update "${stage}" "running"
  set +e
  {
    printf '[run_formal_14b_t2v] command:'
    printf ' %q' "$@"
    printf '\n'
    "$@"
  ...
  if [[ "${status}" != "0" ]]; then
    ...
    printf '[run_formal_14b_t2v] stage %s failed with %s\n' "${stage}" "${status}" >&2
    exit "${status}"
  fi
  date > "${done_marker}"
  state_update "${stage}" "done"
  printf '[run_formal_14b_t2v] done stage %s\n' "${stage}"
  if [[ -n "${STOP_AFTER}" && "${STOP_AFTER}" == "${stage}" ]]; then
    printf '[run_formal_14b_t2v] stopped after %s\n' "${stage}"
    exit 0
  fi
}
...
run_stage subset subset.log "${PY_CMD[@]}" "${T2V_SCRIPT_DIR}/01_make_train_subset.py" --config "${CONFIG}" --run-dir "${RUN_DIR}"
...
printf '[run_formal_14b_t2v] complete: %s\n' "${RUN_DIR}"
