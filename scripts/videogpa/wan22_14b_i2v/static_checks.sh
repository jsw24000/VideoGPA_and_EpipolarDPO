...
vgm_require_profile
CONFIG="${CONFIG:-${VGM_REPO_ROOT}/configs/videogpa/wan22_14b_i2v_formal.yaml}"
...
"${PY_CMD[@]}" -m py_compile \
  ...
  "${I2V_SCRIPT_DIR}/01_make_train_subset.py" \
...
"${PY_CMD[@]}" "${I2V_SCRIPT_DIR}/01_make_train_subset.py" --help >/dev/null
...
if "${PY_CMD[@]}" -c "import torch, peft, PIL" >/dev/null 2>&1; then
  ...
  printf 'static checks: skipped heavy WAN help probes because torch/peft/PIL is not importable in this Python.\n'
fi
...
test -f "${CONFIG}"
test -f "${VGM_REPO_ROOT}/configs/videogpa/wan22_14b_i2v_formal.yaml"
...
"${PY_CMD[@]}" -m vgm_common.config --config "${CONFIG}" --print output_root | grep -F "/videogpa/wan22_14b_i2v/formal" >/dev/null
