...
mkdir -p "${RUN_DIR}/"{config,manifests,candidates,logs,reports,samples,evaluation}
...
run_stage() {
  ...
  if [[ -f "${done_marker}" && "${FORCE_STAGE}" != "${stage}" ]]; then
    printf '[run_formal_generation_i2v] skip done stage %s\n' "${stage}"
    return
  fi
  printf '[run_formal_generation_i2v] start stage %s\n' "${stage}"
  state_update "${stage}" "running"
  set +e
  {
    printf '[run_formal_generation_i2v] command:'
    printf ' %q' "$@"
    printf '\n'
    "$@"
  ...
  if [[ "${status}" != "0" ]]; then
    ...
    printf '[run_formal_generation_i2v] stage %s failed with %s\n' "${stage}" "${status}" >&2
    exit "${status}"
  fi
  date > "${done_marker}"
  state_update "${stage}" "done"
  printf '[run_formal_generation_i2v] done stage %s\n' "${stage}"
  if [[ -n "${STOP_AFTER}" && "${STOP_AFTER}" == "${stage}" ]]; then
    printf '[run_formal_generation_i2v] stopped after %s\n' "${stage}"
    exit 0
  fi
}

FORCE_ENV=0
if [[ "${FORCE_STAGE}" == "generation_candidates" ]]; then
  FORCE_ENV=1
fi
...
if [[ "${RESUME}" == "1" ]]; then
  printf '[run_formal_generation_i2v] resume requested for %s\n' "${RUN_DIR}"
fi
...
printf '[run_formal_generation_i2v] complete: %s\n' "${RUN_DIR}"
