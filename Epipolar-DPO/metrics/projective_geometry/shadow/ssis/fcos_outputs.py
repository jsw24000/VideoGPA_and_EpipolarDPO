...
class FCOSOutputs(nn.Module):
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, offset_pred, locations, gt_instances, top_feats=None,
               top_feats2=None):
        """
        ...
        instances.offset_pred = cat([
            x.permute(0, 2, 3, 1).reshape(-1, 2) for x in offset_pred
        ])
        ...
        if len(top_feats2) > 0:
            instances.top_feats2 = cat([
                # Reshape: (N, -1, Hi, Wi) -> (N*Hi*Wi, -1)
                x.permute(0, 2, 3, 1).reshape(-1, x.size(1)) for x in top_feats2
            ], dim=0, )
    ...
    def predict_proposals(
            self, logits_pred, reg_pred, ctrness_pred, offset_pred,
            locations, image_sizes, top_feats=None, top_feats2=None
    ):
        if self.training:
        ...
        bundle = {
            ...
            "s": self.strides, "offset": offset_pred,
        }

        if len(top_feats) > 0:
            bundle["t"] = top_feats
        if len(top_feats2) > 0:
            bundle["t2"] = top_feats2
        ...
        for i, per_bundle in enumerate(zip(*bundle.values())):
            ...
            c = per_bundle["c"]
            offset = per_bundle["offset"]
            t = per_bundle["t"] if "t" in bundle else None
            t2 = per_bundle["t2"] if "t2" in bundle else None

            sampled_boxes.append(
                self.forward_for_single_feature_map(
                    l, o, r, c, offset, image_sizes, t, t2
                )
            )
    ...
    def forward_for_single_feature_map(
            ...
            ctrness_pred, offset_pred, image_sizes, top_feat=None, top_feat2=None
    ):
        ...
        offset_pred = offset_pred.view(N, 2, H, W).permute(0, 2, 3, 1)
        offset_pred = offset_pred.reshape(N, -1, 2)
        if top_feat is not None:
            top_feat = top_feat.view(N, -1, H, W).permute(0, 2, 3, 1)
            top_feat = top_feat.reshape(N, H * W, -1)

        if top_feat2 is not None:
            top_feat2 = top_feat2.view(N, -1, H, W).permute(0, 2, 3, 1)
            top_feat2 = top_feat2.reshape(N, H * W, -1)
        ...
        for i in range(N):
            ...
            per_offset = offset_pred[i]
            per_offset = per_offset[per_box_loc]

            if top_feat is not None:
                per_top_feat = top_feat[i]
                per_top_feat = per_top_feat[per_box_loc]
            if top_feat2 is not None:
                per_top_feat2 = top_feat2[i]
                per_top_feat2 = per_top_feat2[per_box_loc]
            ...
            if per_candidate_inds.sum().item() > per_pre_nms_top_n.item():
                ...
                if top_feat2 is not None:
                    per_top_feat2 = per_top_feat2[top_k_indices]
            ...
            boxlist.offset_pred = per_offset
            if top_feat is not None:
                boxlist.top_feat = per_top_feat
            if top_feat2 is not None:
                boxlist.top_feat2 = per_top_feat2
