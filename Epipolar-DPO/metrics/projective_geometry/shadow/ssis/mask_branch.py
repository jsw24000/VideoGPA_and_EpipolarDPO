...
class MaskBranch(nn.Module):
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        if self.sem_loss_on:
            ...
            self.logits = nn.Conv2d(channels, 1, kernel_size=1, stride=1)
    ...
    def forward(self, features, gt_instances=None):
        ...
        if self.training and self.sem_loss_on:
            ...
            for per_im_gt in gt_instances:
                n,h,w = per_im_gt.gt_bitmasks_full.size()
                half = int(n/2)
                shadow_mask = per_im_gt.gt_bitmasks_full[half:].sum(0)
                semantic_targets.append(shadow_mask)
            ...
            one_hot = semantic_targets


            loss_sem = nn.functional.binary_cross_entropy_with_logits(logits_pred,one_hot) * 10
