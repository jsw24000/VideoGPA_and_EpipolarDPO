...
def load_samples(input_json: Path) -> list[dict[str, Any]]:
    ...
    for idx, item in enumerate(samples):
        ...
        if not any(key in item for key in ["image_path", "image_prompt", "input_image_path", "first_frame_path"]):
            if not (item.get("scene_id") and item.get("source_bucket")):
                raise ValueError(f"I2V input must include an image field or resolvable scene metadata: index {idx}")
        task = str(item.get("task", "i2v")).lower()
        if task and task != "i2v":
            raise ValueError(f"I2V input must not include non-I2V task={task!r}: {item.get('group_id', idx)}")
        clean.append(item)
    return clean
...
def generate(args: argparse.Namespace) -> None:
    ...
    assert cfg["project"].get("task") == "i2v", "This entrypoint is I2V only"
    ...
    max_area = int(args.max_area or generation_cfg.get("max_area", size[0] * size[1]))
    ...
    print(f"  first_frames_root={cfg['paths']['first_frames_root']}")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("WAN2.2 generation requires CUDA; preflight should report GPU availability first.")
    ...
    for idx, item in enumerate(samples):
        ...
        image_path = resolve_image_path(item, cfg, run_dir)
        ...
        for seed in seeds:
            ...
            if existing_video_ok(video_path, frame_num) and not args.force:
                ...
                print(f"[{idx + 1}/{len(samples)}] I2V generating group={group_id} seed={seed} image={image_path}")
                try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Wan2.2-TI2V-5B I2V generation for VideoGPA")
